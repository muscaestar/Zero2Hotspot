...
import xyz.muscaestar.zero2hp.bytecode.enums.access.FieldAccMask;
...
public class FieldInfo extends ClassItem {
    ...
    public String meta(Function<Short, String> cpoolFunc) {
        final String flags = Arrays.stream(FieldAccMask.resolve(access_flags)).map(Enum::name).collect(Collectors.joining(","));
